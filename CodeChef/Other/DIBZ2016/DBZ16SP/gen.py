...
#	print T
	for t in xrange(T):
#		n = randrange(2, N+1)
#		m = randrange(n-1, min(1+int((n*(n+1))/3.5), M))
		n = N
#		m = randrange(N-1, N)
#		m = N+10
		m = n-1
		...
		graph = genRandomTree(n)		
#		graph = genRandomConnectedGraph(n, m)
		labels = genRandomString(m, 'a', 'z')
		for i in xrange(m):
			print graph[i][0], graph[i][1], labels[i]
