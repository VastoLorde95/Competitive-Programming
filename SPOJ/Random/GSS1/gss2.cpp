...
class SegmentTree {
	...
	SegmentTree(T arr[], int N) {
		...
		/*for(int i = 1; i <= getSegmentTreeSize(N); i++){
			printf("%d %d %d %d\n",nodes[i].prefixMaxSum, nodes[i].suffixMaxSum, nodes[i].maxSum, nodes[i].sum);
			//cout<<nodes[i].prefixMaxSum<<" "<<nodes[i].suffixMaxSum<<" "<<nodes[i].maxSum<<" "<<nodes[i].sum<<endl;
		}*/
