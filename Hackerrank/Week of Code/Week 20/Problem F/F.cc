...
void build(int x, int v){
	for(x += d-1; x; x >>= 1) segmentTree[x].add(v);
}

void update(int x, int y, int v){
	for(x += d-1; x; x >>= 1) segmentTree[x].update(y, v);
}

int get(int x, int y){
	int res = INF;
	for(int p = x + d-1, q = y + d-1; p < q ; p >>= 1, q >>= 1){
		if((p&1)) res = min(res, segmentTree[p++].query(y-1));
		if((q&1)) res = min(res, segmentTree[--q].query(y-1));
	}
	return res;
}
...
int main(){
	...
	for(int i = 1; i <= d+d; i++) segmentTree[i].init();
