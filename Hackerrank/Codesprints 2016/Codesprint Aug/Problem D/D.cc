...
void solve(){
	...
	for(int i = 0; i < n; i++){
		if(a[i] >= 'A' and a[i] <= 'Z'){
			cnt++;
			for(int j = 0; j < m; j++){
				if(a[i] == b[j]) cost[i+1][j+1] = -n;
				else cost[i+1][j+1] = INF;
			}
		}
		else{
			for(int j = 0; j < m; j++){
				if(a[i] - 32 == b[j]) cost[i+1][j+1] = 0;
				else cost[i+1][j+1] = INF;
			}
		}
		for(int j = m+1; j <= t; j++){
    ...
    ll ans = minCostBipartiteMatching();
//    tr(matchingCount, ans, ans - INF * (n-m), n-m, INF);
	if(ans - INF * (n-m) == - 1ll * cnt * n) puts("YES"); else puts("NO");
